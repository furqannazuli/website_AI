...
def preprocess_fraudulent(data, scaler):
    ...
    data['Customer Age'] = imputer.fit_transform(data[['Customer Age']])
...
def process_chat():
    ...
    headers = {
        'Content-Type': 'application/json',  # Define headers with content type
    }
    # Sending POST request to Rasa server
    try:
        rasa_response = requests.post('http://host.docker.internal:5005/webhooks/rest/webhook', json=data, headers=headers)
        return jsonify(rasa_response.json()), rasa_response.status_code
